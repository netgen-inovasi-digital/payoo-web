...
const fetchStocks = async () => {
  try {
    ...
    if (response.status === 'success') {
      ...
      perPage.value = response.meta?.pagination?.per_page || 20
      totalItems.value = response.meta?.pagination?.total || 0
      totalPages.value = response.meta?.pagination?.total_pages || 0
    }
  } catch (error) {
...
watch([selectedProductId, selectedType, dateFrom, dateTo], () => {
  currentPage.value = 1
  fetchStocks()
})
